...
def analyzeResponse(adb, response) -> Tuple[bool, str, dict]:
    ...
    result = makeAction(), toolCall.function.name, toolArgs
...
def actionScript(adb: AdbClient, ai: AiManager, action):
    ...
    # do an action
    last_action_status, last_function_name, last_function_args = analyzeResponse(adb, res)
...
def main():
    ...
    ai = AiManager(
        apiKey='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',
        systemPrompt=SYSTEM_PROMPT
    )
    adb = m.loadSerial('98896a374256375548')

    actionScript(
        adb, ai, 'open page instagram.com'
    )
